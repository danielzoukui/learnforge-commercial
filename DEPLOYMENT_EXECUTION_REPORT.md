...
**Distribution artifact:** `LearnForge_COMMERCIAL_MONETIZATION_COMPLETE_v17.2.zip` — 67 files, 2,358,127 bytes,
SHA-256 `fea7aba1318ca331f72bf6404db2a06e1cda54b309b7abf8e7188c7d773b83f0`
...
`npm run test:golive` — **17 checks, all passing** — runs `scripts/golive.mjs` against mock Northflank,
...
✓ readiness checker warns when email confirmation will block the rehearsal
✓ purchase step prints the exact recovery command when confirmation blocks sign-up

GO-LIVE PIPELINE SUITE PASSED (17 checks)
```
...
| Go-live pipeline suite against mock provider APIs | **17/17 checks**, real app + real PostgreSQL + real HMAC |
| CI | new real-PostgreSQL job; both jobs green on PR #2 |
| Full suite | **62 assertions + 13 end-to-end checks + 17 pipeline checks**, `npm run check` clean |
| Release artifact | `…_v17.2.zip`, 67 files, sha256 `fea7aba1…3b83f0` — now ships `scripts/golive*.mjs` and the pipeline suite |
